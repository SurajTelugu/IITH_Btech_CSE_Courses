...
/* This function is used to read input from file and get simulations for graph.
// global file pointer
FILE* fp;
...
int main()
{
  ...
  scanf("%u",&num_of_threads);
  // Getting the source point in given format
  source = getpoint(source);
  ...
  //fp = fopen("data.txt","r"); Reading file

  // Taking input points from user
  coordn pts[num_of_pts];
  for(int i=0;i<num_of_pts;i++)
  {
    pts[i] = getpoint(pts[i]); // Getting points from user in given format
